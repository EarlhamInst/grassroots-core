...
bool InitServiceJob (ServiceJob *job_p, Service *service_p, const char *job_name_s, const char *job_description_s, bool (*update_fn) (struct ServiceJob *job_p), bool (*calculate_results_fn) (struct ServiceJob *job_p), void (*free_job_fn) (struct ServiceJob *job_p), uuid_t *id_p, const char *job_type_s)
{
	...
	if (id_p)
		{
			PrintLog (STM_LEVEL_FINER, __FILE__, __LINE__, "InitServiceJob () for \"%s\" with name \"%s\" calling uuid_copy", GetServiceName (service_p), job_name_s ? job_name_s : "NULL");
		...
		{
			PrintLog (STM_LEVEL_FINER, __FILE__, __LINE__, "InitServiceJob () for \"%s\" with name \"%s\" calling uuid_generate ()", GetServiceName (service_p), job_name_s ? job_name_s : "NULL");
